/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package dao;

import java.io.BufferedReader;
import java.io.BufferedWriter;
import java.io.File;
import java.io.FileReader;
import java.io.FileWriter;
import java.io.IOException;
import java.io.PrintWriter;
import sng.FacilityBookingManagementSng;


/**
 *
 * @author User
 */
public class FacilityBookingManagementDaoImpl implements FacilityBookingManagementDao{
    public boolean facilityBookingManagement(FacilityBookingManagementSng fbms) {
        try {
            if (!"".equals(fbms.getUserID()) && !"".equals(fbms.getFacility()) && !"".equals(fbms.getDateIn()) && !"".equals(fbms.getDuration()) && !"".equals(fbms.getTime())){
                File f = new File("src/textFiles/facilityBooking.txt");
                if (!f.exists()) {
                    f.createNewFile();
                }
                BufferedReader br = new BufferedReader(new FileReader(f));
                Object[] Lines = br.lines().toArray();
                int i = 0;
                int faciBookingid = 0;
                for (i = 1; i < Lines.length; i++) {
                    String line = Lines[i].toString().trim();
                    String[] row = line.split(",");
                    faciBookingid = Integer.parseInt(row[0]);

                }
                int faciBookingID = faciBookingid + 1;
                FileWriter fw = new FileWriter(f, true);
                BufferedWriter bw = new BufferedWriter(fw);
                PrintWriter pw = new PrintWriter(bw);
                pw.println(faciBookingID + "," + fbms.getUserID() + "," + fbms.getFacility() + "," + fbms.getDateIn() + "," + fbms.getDuration() + "," + fbms.getTime() + ",");
                pw.flush();
                pw.close();
                bw.close();
                return true;
            } else{
                return false;
            }

        } catch (IOException e) {
            System.out.println("FileNotFound");
        }

        return false;

    }
}
